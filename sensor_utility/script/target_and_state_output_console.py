...
class TopicSubscirber:
	...
	def subscribe_tpics(self):
		...
		rospy.Subscriber("input_devision", Int8, self.devisionCallback)
		rospy.Subscriber("arduino_input",String,self.arduinoCallback)
		rospy.Subscriber("arduino_recive",String,self.reciveCallback)
	...
	def console_print(self):
		...
		print "time,command_angle_deg,state_angle_deg,devision,arduino_command,arduino_recive,state_vel"
		
		while not rospy.is_shutdown():
			now = rospy.get_rostime()
			print "%d.%09d,%f,%f,%f,%s,%s,%f,%f" % ( now.secs - start_time.secs ,now.nsecs , self.seniorcar_command.steer_angle ,self.seniorcar_state.steer_angle ,self.devision.data ,self.arduino_str.data,self.recive_str.data,self.seniorcar_command.vehicle_velocity,self.seniorcar_state.vehicle_velocity)
			rate.sleep()
